...
/// Values for `from_mont` that exercise the shared reduction
/// helper's rare carry propagations.
///
/// These and the operand pairs below are regenerated by
/// `python3 sage/asm_mutant_witnesses.py`, which prints both arrays in this
/// shape. `cargo xtask asm-mutants` is what identified the need for them, and
/// sage/README.md explains why they have to be constructed rather than
/// searched for: each drives a carry that random operands reach with
/// probability about 2^-64.
const AARCH64_ASM_CARRY_REDUCTIONS: [[u64; 4]; 6] = [
    [
        0x0000000000000007,
        0x29ef6ceb411acfbc,
        0xffffffffffffffff,
...
/// Operand pairs for `mul` that exercise carry propagations in the
/// AArch64 backend which random testing does not reach.
///
/// Some left operands are deliberately not canonical: `from_u512` multiplies
/// an arbitrary 256-bit value by `R2` or `R3`, and that is what makes the
/// per-round fifth accumulator limb reachable.
const AARCH64_ASM_CARRY_PRODUCTS: [([u64; 4], [u64; 4]); 8] = [
    (
        [
            0x325a61da00000002,
            0xcda6461167ef478b,
...
#[test]
fn aarch64_asm_matches_portable_at_carry_boundaries() {
    // Each vector drives a carry propagation that the boundary and random
    // cases above never reach; see the notes on the two arrays for how they
    // were derived and how to regenerate them.
    for (lhs, rhs) in AARCH64_ASM_CARRY_PRODUCTS {
        assert_eq!(
            super::aarch64_asm::mul(&lhs, &rhs, &MODULUS.0, INV),
            Fp::mul(&Fp(lhs), &Fp(rhs)).0,
        );
    }

    for value in AARCH64_ASM_CARRY_REDUCTIONS {
        assert_eq!(
            super::aarch64_asm::from_mont(&value, &MODULUS.0, INV),
            Fp::montgomery_reduce(value[0], value[1], value[2], value[3], 0, 0, 0, 0).0,
        );
        assert_eq!(
            super::aarch64_asm::square(&value, &MODULUS.0, INV),
            Fp::mul(&Fp(value), &Fp(value)).0,
        );
    }
}
